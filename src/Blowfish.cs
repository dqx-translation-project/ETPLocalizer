...
internal sealed class DqxBlowfish
{
    ...
    private const int PSz     = Rounds + 2;
    ...
    private readonly uint[] _s1 = new uint[SboxSk];
    private readonly uint[] _s2 = new uint[SboxSk];
    private readonly uint[] _s3 = new uint[SboxSk];
    private readonly uint[] _p  = new uint[PSz];
    ...
    {
        if (key.Length < 1)
            throw new ArgumentException("Blowfish key cannot be empty");
    ...
    public byte[] Transform(byte[] input)
    {
        ...
        {
            int idx = i * 8;
            if (_encrypting) EncryptBlock(input, idx, output, idx);
            else             DecryptBlock(input, idx, output, idx);
        }
        return output;
    }

    private uint F(uint x) =>
        ((_s0[x >> 24] + _s1[(x >> 16) & 0xFF]) ^ _s2[(x >> 8) & 0xFF]) + _s3[x & 0xFF];
    ...
    {
        uint xl = BinaryPrimitives.ReadUInt32LittleEndian(input.AsSpan(inOff));
        uint xr = BinaryPrimitives.ReadUInt32LittleEndian(input.AsSpan(inOff + 4));
        xl ^= _p[0];
        for (int i = 1; i < Rounds; i += 2)
        {
            xr ^= F(xl) ^ _p[i];
        ...
        xr ^= _p[Rounds + 1];
        BinaryPrimitives.WriteUInt32LittleEndian(output.AsSpan(outOff),     xr);
        BinaryPrimitives.WriteUInt32LittleEndian(output.AsSpan(outOff + 4), xl);
    }
